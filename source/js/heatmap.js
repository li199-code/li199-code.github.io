...
try {
    if (!['http://localhost:4000/', 'https://blog.jasonleehere.com/'].includes(window.location.href)) throw new Error('not in index');
    ...
    fetch(bloginfoPath)
        ...
        .then(data => {
            if (!['http://localhost:4000/', 'https://blog.jasonleehere.com/'].includes(window.location.href)) return;
